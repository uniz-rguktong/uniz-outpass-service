  ...
  try {
    ...
    const outpass = await prisma.outpass.create({
      data: {
        ...
        studentGender: (
          gender || String(req.body.studentGender || "M")
        ).toUpperCase(),
        reason,
        fromDay: fromDateObj,
        toDay: toDateObj,
        ...
        approvalLogs: [],
        currentLevel:
          (gender || req.body.studentGender) === "F"
...
export const createOuting = async (
  ...
  try {
    ...
    const outing = await prisma.outing.create({
      data: {
        studentId: user.username.toUpperCase(),
        studentGender: (gender || req.body.studentGender || "M").toUpperCase(),
        reason,
...
export const approveOutpass = async (
  ...
  try {
    ...
    if (!isSuper) {
      if (
        (user.role === UserRole.CARETAKER_FEMALE ||
